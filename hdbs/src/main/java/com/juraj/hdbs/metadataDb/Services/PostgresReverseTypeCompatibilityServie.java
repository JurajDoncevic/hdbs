...
/** Service class used to manage data type compatibilities for a PostgreSQL database stored in a metadata database
 * @author Juraj
 */
public class PostgresReverseTypeCompatibilityServie {
     ...
     */
    public PostgresReverseTypeCompatibilityServie(String connectionUrl){
    ...
    public boolean insertCompatibility(String typeName1, String typeName2){
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.execute(String.format("INSERT INTO pg_reverse_type_compatibility(type_name_src, type_name_dest) VALUES('%s', '%s');", typeName1, typeName2));
    ...
    public boolean deleteCompatibility(String typeName1, String typeName2){
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.execute(String.format("DELETE FROM pg_reverse_type_compatibility WHERE type_name_src = '%s' AND type_name_dest = '%s' OR type_name_src = '%s' AND type_name_dest = '%s';", typeName1, typeName2, typeName2, typeName1));

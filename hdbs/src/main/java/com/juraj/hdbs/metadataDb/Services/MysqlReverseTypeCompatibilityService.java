...
/** Service class used to manage data type compatibilities for a MySQL database stored in a metadata database
 * @author Juraj
 */
public class MysqlReverseTypeCompatibilityService {
     ...
     */
    public MysqlReverseTypeCompatibilityService(String connectionUrl){
    ...
    public List<Pair<String, String>> getAllCompatibilities(){
        ...
        try {
            ...
            statement.execute("SELECT * FROM 'mysql_reverse_type_compatibility';");
    ...
    public List<String> getCompatibleTypesForType(String typeName){
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.execute(String.format("SELECT type_name_dest " +
                                            "FROM mysql_reverse_type_compatibility " +
                                            "WHERE type_name_src = '%s' " +
                                            "UNION " +
                                            "SELECT type_name_src " +
                                            "FROM mysql_reverse_type_compatibility " +
                                            "WHERE type_name_dest = '%s';",  typeName, typeName));
    ...
    public boolean insertCompatibility(String typeName1, String typeName2){
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.execute(String.format("INSERT INTO mysql_reverse_type_compatibility(type_name_src, type_name_dest) VALUES('%s', '%s');", typeName1, typeName2));
    ...
    public boolean deleteCompatibility(String typeName1, String typeName2){
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.execute(String.format("DELETE FROM mysql_reverse_type_compatibility WHERE type_name_src = '%s' AND type_name_dest = '%s' OR type_name_src = '%s' AND type_name_dest = '%s';", typeName1, typeName2, typeName2, typeName1));
